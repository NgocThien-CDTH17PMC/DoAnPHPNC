		...
		<div class="card">
			<div class="card-body">
				...
				<table id="thung-rac-datatable" class="table nowrap">
					...
					<tbody>
						@foreach($dsLinhVucDaXoa as $linhvuc)
						<tr>
							<td>{{$linhvuc->id}}</td>
							<td>{{$linhvuc->ten_linh_vuc}}</td>
							<td>
								<form action="{{ route('linh-vuc.xoa-linh-vuc-di', $linhvuc->id) }}" method="POST">
									@method('DELETE')
									@csrf
									<a href="{{ route('linh-vuc.phuc-hoi-linh-vuc', $linhvuc->id) }}" class="btn btn-info waves-effect waves-light">
				...
				<a href="{{ route('linh-vuc.ds-linh-vuc') }}" class="btn btn-info waves-effect waves-light">
					Trở về
				</a>
			</div> <!-- end card body-->
		</div> <!-- end card -->
